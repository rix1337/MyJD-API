...
import myjd.myjdapi
from myjd.common import is_device
from myjd.common import readable_size
from myjd.common import readable_time
...
def get_device(configfile):
    ...
    if myjd_user and myjd_pass and myjd_device:
        ...
        except myjd.myjdapi.MYJDException as e:
...
def check_failed_packages(configfile, device):
    try:
        ...
        if device:
            ...
            except myjd.myjdapi.TokenExpiredException:
                device = get_device(configfile)
                if not device or not is_device(device):
                    return False
                grabber_collecting = device.linkgrabber.is_collecting()
